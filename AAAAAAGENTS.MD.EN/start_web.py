...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Start AGENTS local visualization service")
    parser.add_argument("--host", default=DEFAULT_HOST, help=f"Listening address, default {DEFAULT_HOST}")
    parser.add_argument("--port", type=int, default=DEFAULT_PORT, help=f"Listening port, default {DEFAULT_PORT}")
    parser.add_argument("--no-reload", action="store_true", help="Turn off uvicorn automatic reload")
    parser.add_argument("--skip-install", action="store_true", help="Skip dependency installation check")
    parser.add_argument("--verbose", action="store_true", help="Output debug log")
