...
def resolve_doc(name: str) -> Path:
    ...
    if normalized not in all_docs_order():
        raise HTTPException(status_code=404, detail=f"Unknown document: {normalized}")
    path = (ROOT / normalized).resolve()
    if path != ROOT and ROOT not in path.parents:
        raise HTTPException(status_code=400, detail="Document path out of bounds")
    if not path.exists():
        raise HTTPException(status_code=404, detail=f"Document does not exist: {normalized}")
    return path
...
def build_verify_hints(errors: list[str]) -> list[str]:
    hints: list[str] = []
    if any("There must be and can only be 1 node status change" in err for err in errors) or any(
        "exactly one node status change is required" in err for err in errors
    ):
        hints.append("This final inspection requires that the milestone status must and can only have one unfinished->done change")
        hints.append('If you are currently only doing document/UI maintenance, please click "Synchronize final inspection baseline" first and then perform final inspection')
    if any("CHANGE.md: One change record must and can only be added" in err for err in errors) or any(
        "CHANGE.md: exactly one new change entry is required" in err for err in errors
    ):
        hints.append("Please add only the latest record in CHANGE.md, historical entries cannot be modified or deleted")
    if any("read-only file modified" in err for err in errors) or any("read-only file modified" in err for err in errors):
        hints.append('It is detected that the user has modified the read-only baseline file, first execute "Synchronize final inspection baseline" to set the current content as the new standard')
    if any("date format error" in err for err in errors) or any("invalid date" in err for err in errors):
        hints.append("CHANGE.date needs to use YYYY-MM-DD-HH-MM format, such as 2026-03-09-15-30")
    if not hints and errors:
        hints.append("Please fix the errors one by one according to the error list before performing the final inspection. If necessary, synchronize the final inspection baseline first")
    return hints
...
def update_node(name: str, node_id: str, fields: dict[str, Any]) -> dict[str, Any]:
    ...
    nodes = shape["nodes"]
    if not isinstance(nodes, list):
        raise HTTPException(status_code=400, detail="The node list format is incorrect")
...
def create_milestone_node(payload: dict[str, Any], node_id: str, title: str) -> dict[str, Any]:
    nodes = payload.get("milestones")
    if not isinstance(nodes, list):
        raise HTTPException(status_code=400, detail="Milestone data format is incorrect")
    if any(isinstance(one, dict) and str(one.get("id")) == node_id for one in nodes):
        raise HTTPException(status_code=409, detail=f"Milestone ID already exists: {node_id}")
    node = {
        "id": node_id,
...
def delete_milestone_node(payload: dict[str, Any], node_id: str) -> dict[str, Any]:
    ...
    if len(kept) == len(nodes):
        raise HTTPException(status_code=404, detail=f"Node not found: {node_id}")
    if not kept:
        raise HTTPException(status_code=400, detail="Reserve at least one milestone node")
    for one in kept:
...
def standard_relpath_from_name(raw: str) -> str:
    ...
    if value.startswith("agents_standards/"):
        ...
        if not filename:
            raise HTTPException(status_code=400, detail="Invalid standard file name")
...
def save_doc(name: str, body: SaveDocRequest) -> JSONResponse:
    ...
    if not is_user_editable_doc(normalized):
        raise HTTPException(status_code=403, detail=f"{normalized} is a read-only document")
    backup = backup_file(path)
    path.write_text(body.content, encoding="utf-8")
    return JSONResponse(
        {
            "status": "ok",
            "message": f"{normalized} saved",
            "backup": backup.relative_to(ROOT).as_posix(),
            "last_modified": mtime_stamp(path),
        }
    )
...
def reset_doc(name: str) -> JSONResponse:
    ...
    if backup is None:
        raise HTTPException(status_code=404, detail=f"No backup of {normalized} found")
    path.write_text(backup.read_text(encoding="utf-8"), encoding="utf-8")
    return JSONResponse(
        {
            "status": "ok",
            "message": f"{normalized} restored from backup",
...
def get_model(name: str) -> JSONResponse:
    ...
    if normalized not in MODEL_EDITABLE_DOCS:
        raise HTTPException(status_code=403, detail=f"{normalized} does not support structured node editing")
    path = resolve_doc(normalized)
    doc = load_document(path)
...
def create_model_node(name: str, body: NodeCreateRequest) -> JSONResponse:
    ...
    if normalized != "MILESTONE.md":
        raise HTTPException(status_code=403, detail="Only MILESTONE supports node addition and deletion")
...
def create_standard(body: StandardCreateRequest) -> JSONResponse:
    ...
    if path.exists():
        raise HTTPException(status_code=409, detail=f"Standard already exists: {path.name}")
...
def delete_standard(name: str) -> JSONResponse:
    ...
    if rel not in list_standard_docs():
        raise HTTPException(status_code=404, detail=f"Standard does not exist: {rel}")
    path = (ROOT / rel).resolve()
    if not path.exists():
        raise HTTPException(status_code=404, detail=f"Standard does not exist: {rel}")
    path.unlink()
...
def verify_finalize() -> JSONResponse:
    ...
    parsed: dict[str, Any]
    try:
        parsed = json.loads(stdout) if stdout else {"status": "failed", "errors": ["The final inspection script has no output content"]}
    except json.JSONDecodeError:
        parsed = {"status": "failed", "errors": [stdout or "The final script output is not valid JSON"]}
...
def _refresh_baseline_impl() -> JSONResponse:
    ...
    payload: dict[str, Any]
    try:
        payload = json.loads(stdout) if stdout else {"status": "failed", "message": "The baseline script has no output"}
    except json.JSONDecodeError:
        payload = {"status": "failed", "message": stdout or "Baseline script output is not valid JSON"}
...
def save_graph_layout(name: str, body: GraphLayoutRequest) -> JSONResponse:
    ...
    if normalized not in GRAPH_DOCS:
        raise HTTPException(status_code=403, detail=f"{normalized} does not support flowchart layout persistence")
    path = resolve_doc(normalized)
    doc = load_document(path)
    payload = dict(doc["payload"])
