    ...
    function docKindLabel(kind) {
      if (kind === "graph_doc") return "流程图";
      if (kind === "tree_doc") return "树视图";
      return "文档";
    }
    ...
    async function fetchJson(url, options = {}) {
      ...
      if (!resp.ok) {
        const message = data && (data.detail || data.message || data.stderr) ? (data.detail || data.message || data.stderr) : `请求失败: ${resp.status}`;
    ...
    function showVerifyModal(payload) {
      ...
      verifyModalSummary.innerHTML = `
        <div class="alert ${status === "ok" ? "alert-success" : "alert-danger"} mb-0">
          状态: ${escapeHtml(status === "ok" ? "通过" : "失败")} | 返回码: ${escapeHtml(code)} | 错误数: ${escapeHtml(errorCount)}
        </div>
      `;
    ...
    function renderNavTree() {
      ...
      docCountEl.textContent = `${fileCount} 文件`;
    }

    function updateDocHead() {
      if (!state.currentDoc) {
        docTitleEl.textContent = "文档预览";
        docMetaEl.innerHTML = "";
        return;
      }
      ...
      docMetaEl.innerHTML = `
        ...
        <span class="file-tag tag-plain">用户可编辑</span>
        <span class="file-tag tag-plain">${state.uiMode === "edit" ? "编辑模式" : "预览模式"}</span>
      `;
    }

    function updateStatePills() {
      if (!state.currentDoc) {
        docStatePill.textContent = "未加载文档";
      } else {
        docStatePill.textContent = `${state.currentDoc} | ${docKindLabel(state.currentDocKind)} | 用户可编辑 | ${state.uiMode === "edit" ? "编辑模式" : "预览模式"}`;
      }
    }
    ...
    function updateButtons() {
      ...
      toggleModeBtn.textContent = inEdit ? "返回预览" : "进入编辑";
    ...
    function resetGraphDetail() {
      const html = `<h3 class="detail-title">节点详情</h3><div class="text-muted">点击节点查看结构化字段</div>`;
    ...
    function renderGraphDetail(raw) {
      ...
      let html = `<h3 class="detail-title">节点详情</h3>`;
    ...
    function renderGraphList(graph) {
      ...
      if (!nodes.length) {
        graphListEl.innerHTML = "<div class='text-muted'>当前无节点</div>";
        if (graphListDetail) graphListDetail.innerHTML = "<div class='text-muted'>当前无节点详情</div>";
        return;
      }
    ...
    function renderGraph(graph) {
      ...
      try {
        ...
        setRuntimeStatus(`流程图渲染失败: ${error.message || error}`, true);
        return;
      }
      resetGraphDetail();
      ...
      state.cy.on("dragfree", "node", () => {
        ...
        setRuntimeStatus("检测到节点位置变化，请点击“保存布局”持久化");
        updateButtons();
      });
      focusGraph();
      updateButtons();
    ...
    function renderTreeDetail() {
      ...
      if (!node) { treeDetail.innerHTML = "<div class='text-muted'>点击左侧节点查看详情</div>"; return; }
      treeDetail.innerHTML = `
        <dl class="mb-0">
          <dt>路径</dt><dd class="mono">${escapeHtml(node.path)}</dd>
          <dt>类型</dt><dd>${escapeHtml(node.type)}</dd>
          <dt>状态</dt><dd>${escapeHtml(node.status)}</dd>
          <dt>最后修改</dt><dd class="mono">${escapeHtml(node.last_modified)}</dd>
          <dt>可编辑</dt><dd>${node.editable ? "是" : "否"}</dd>
          <dt>文件作用</dt><dd>${escapeHtml(node.note || "")}</dd>
        </dl>
      `;
    }
    ...
    function renderTreeExplorer(explorer) {
      ...
      if (!explorer || !explorer.root) {
        treeExplorer.innerHTML = "<div class='text-muted'>TREE 结构为空</div>";
        return;
      }
    ...
    async function loadMeta() {
      try {
        ...
        setRuntimeStatus(`后端已连接 ${version}`);
      } catch (error) {
        setRuntimeStatus(`后端元信息读取失败: ${error.message}`, true, true);
      }
    }
    ...
    async function loadDoc(name) {
      try {
        ...
        if (state.currentDoc === TREE_DOC) {
          ...
          treeExplorer.innerHTML = "";
          treeDetail.innerHTML = "<div class='text-muted'>点击左侧节点查看详情</div>";
        }
        ...
        renderNodeForm();
        updateButtons();
        setRuntimeStatus("文档加载完成");
      } catch (error) {
        setRuntimeStatus(`加载失败: ${error.message}`, true, true);
        updateButtons();
      }
    }
    async function createStandardDoc() {
      const raw = await uiPrompt(
        "新建标准",
        "请输入标准名称（仅字母、数字、下划线），系统将自动追加 _STANDARD.md",
        "例如：PYTHON_STYLE",
        ""
      );
      if (raw === null) return;
      const name = String(raw).trim();
      if (!name) { setRuntimeStatus("标准名称不能为空", true, true); return; }
      try {
    ...
    async function syncTree() {
      try {
        ...
        if (data.exit_code !== 0) throw new Error(data.stderr || data.stdout || "TREE 同步失败");
        setRuntimeStatus("TREE 同步完成", false, true);
        await refreshDocsIndex();
        if (state.currentDoc) await loadDoc(state.currentDoc);
      } catch (error) {
        setRuntimeStatus(`TREE 同步失败: ${error.message}`, true, true);
      }
    }
    ...
    async function verifyFinalize() {
      try {
        ...
        if (data.exit_code !== 0 || (data.error_count || 0) > 0) {
          ...
          const first = Array.isArray(data.errors) && data.errors.length ? data.errors[0] : "未知错误";
          throw new Error(first);
        }
        const fixText = data.fix_count ? `，自动修复 ${data.fix_count} 项` : "";
        setRuntimeStatus(`终检通过${fixText}`, false, true);
        await refreshDocsIndex();
        if (state.currentDoc) await loadDoc(state.currentDoc);
      } catch (error) {
        setRuntimeStatus(`终检失败: ${error.message}`, true, true);
      }
    }
    ...
    async function refreshBaseline() {
      ...
      for (const endpoint of candidates) {
        for (const method of ["POST", "GET"]) {
          try {
            ...
            setRuntimeStatus(`终检基线已同步（${method} ${endpoint}）`, false, true);
            return;
          } catch (error) {
      ...
      const hint = all404 ? "；后端路由不存在，请重启服务并硬刷新页面" : "";
      setRuntimeStatus(`基线同步失败: ${lastError || "未知错误"}${hint}`, true, true);
    }
    ...
    async function saveRaw() {
      ...
      try {
        ...
        setRuntimeStatus(`原文保存成功: ${data.backup}`, false, true);
        await refreshDocsIndex();
        await loadDoc(state.currentDoc);
        applyMode("edit");
      } catch (error) {
        setRuntimeStatus(`原文保存失败: ${error.message}`, true, true);
      }
    }

    async function resetDoc() {
      if (!state.currentDoc || !state.currentEditable || state.uiMode !== "edit") return;
      const ok = await uiConfirm("重置文档", `确认将 ${state.currentDoc} 回退到最近备份版本吗？`, true);
      if (!ok) return;
      try {
        const data = await fetchJson(`/api/docs/${encodePath(state.currentDoc)}/reset`, { method: "POST" });
        setRuntimeStatus(`文档已恢复: ${data.restored_from}`, false, true);
        await refreshDocsIndex();
        await loadDoc(state.currentDoc);
        applyMode("edit");
      } catch (error) {
        setRuntimeStatus(`重置失败: ${error.message}`, true, true);
      }
    }

    async function saveNode() {
      if (!state.currentDoc || !state.currentEditable || state.uiMode !== "edit" || !state.selectedNodeId) {
        setRuntimeStatus("请先在编辑模式选择节点", true, true);
        return;
      }
      ...
      try {
        ...
        setRuntimeStatus("节点保存成功", false, true);
        await refreshDocsIndex();
        await loadDoc(state.currentDoc);
        applyMode("edit");
        ...
        setRuntimeStatus(`节点保存失败: ${error.message}`, true, true);
      }
    }
    async function createMilestoneNode() {
      if (state.currentDoc !== MILESTONE_DOC || state.uiMode !== "edit") return;
      const id = await uiPrompt("新建里程碑节点", "请输入节点 ID", "例如：MS-INIT-010", "");
      if (id === null) return;
      const nodeId = String(id).trim();
      if (!nodeId) { setRuntimeStatus("节点 ID 不能为空", true, true); return; }
      const titleRaw = await uiPrompt("新建里程碑节点", "请输入节点标题（可留空使用 ID）", "节点标题", nodeId);
      ...
      try {
        ...
        setRuntimeStatus(`里程碑节点已创建: ${nodeId}`, false, true);
        await refreshDocsIndex();
        ...
        setRuntimeStatus(`新建节点失败: ${error.message}`, true, true);
      }
    }

    async function deleteMilestoneNode() {
      if (state.currentDoc !== MILESTONE_DOC || state.uiMode !== "edit" || !state.selectedNodeId) return;
      const ok = await uiConfirm("删除里程碑节点", `确认删除 ${state.selectedNodeId} 吗？该操作不可恢复。`, true);
      if (!ok) return;
      ...
      try {
        ...
        state.selectedNodeId = null;
        setRuntimeStatus(`里程碑节点已删除: ${deletingId}`, false, true);
        await refreshDocsIndex();
        await loadDoc(MILESTONE_DOC);
        applyMode("edit");
      } catch (error) {
        setRuntimeStatus(`删除节点失败: ${error.message}`, true, true);
      }
    }
    ...
    async function saveGraphLayout() {
      ...
      try {
        ...
        state.graphLayoutDirty = false;
        setRuntimeStatus(`布局保存成功: ${data.saved_count} 个节点`, false, true);
        updateButtons();
      } catch (error) {
        setRuntimeStatus(`保存布局失败: ${error.message}`, true, true);
      }
    }
    ...
    async function initApp() {
      ...
      if (defaultDoc) {
        ...
        setRuntimeStatus("未发现可加载文档", true, true);
      }
      updateButtons();
    }
